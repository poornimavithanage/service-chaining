...
import com.poornima.rentcloud.rentservice.hystrix.CommonHysctrixCommand;
import com.poornima.rentcloud.rentservice.hystrix.VehicleCommand;
import com.poornima.rentcloud.rentservice.model.DetailResponse;
import com.poornima.rentcloud.rentservice.repository.RentRepository;
...
public class RentServiceImpl implements RentService {
    ...
    public DetailResponse findDetailResponse(int id) throws ExecutionException, InterruptedException {
        ...
        Rent rent = findById(id); //get rent response first
        Customer customer = getCustomer(rent.getCustomerId()); //extract the customerId parameter from rest response
         Vehicle vehicle = getVehicle(rent.getVehicleId()); //extract the vehicleId parameter from rest response

        return new DetailResponse(rent, customer, vehicle); //invoke two endpoints and build the rest response and send it back
